  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      if (referentialIntegrityConflict) {
        setDeleteError(
          'Delete failed. This sub category is still referenced by one or more category/sub category links or linked items. Reassign or remove the linked records first, then try again.'
        );
        return;
      }
          ...
          <div className="mb-8 bg-white p-6 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">
                {editingId !== null ? 'Edit Sub Category' : 'New Sub Category'}
              </h2>
              <button
                onClick={() => {
                  setIsAdding(false);
