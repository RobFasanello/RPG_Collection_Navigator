          ...
          <div className="mb-8 bg-white p-6 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">
                {editingId !== null ? 'Edit Status' : 'New Status'}
              </h2>
              <button
                onClick={() => {
                  setIsAdding(false);
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {Array.isArray(getSortedRecords()) && getSortedRecords().map((record: Status) => (
                <tr key={record.StatusID} className="hover:bg-gray-50">
                  <td className="px-6 py-4">{record.StatusName}</td>
                  <td className="px-6 py-4 text-right space-x-2">
                    <button
                      onClick={() => setEditingId(String(record.StatusID))}
                    ...
                    <button
                      onClick={() => handleDelete(String(record.StatusID))}
