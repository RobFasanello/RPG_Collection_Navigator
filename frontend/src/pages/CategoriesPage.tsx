...
export default function CategoriesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      if (referentialIntegrityConflict) {
        setDeleteError(
          'Delete failed. This category is still referenced by one or more category/sub category links or linked items. Reassign or remove the linked records first, then try again.'
        );
        return;
      }
  ...
  return (
    <AdminLayout title="Categories" subtitle="Use this screen to view, add, remove and modify the categories in your collection.">
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {Array.isArray(getSortedRecords()) && getSortedRecords().map((record: Category) => (
                <tr key={record.CategoryID} className="hover:bg-gray-50">
                  <td className="px-6 py-4">{record.CategoryName}</td>
                  <td className="px-6 py-4 text-right space-x-2">
                    <button
                      onClick={() => setEditingId(String(record.CategoryID))}
                    ...
                    <button
                      onClick={() => handleDelete(String(record.CategoryID))}
