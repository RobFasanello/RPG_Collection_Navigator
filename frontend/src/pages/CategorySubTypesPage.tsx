          ...
          <div className="mb-8 bg-white p-6 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">
                {editingId ? 'Edit Category Subtype' : 'New Category Subtype'}
              </h2>
              <button
                onClick={() => {
                  setIsAdding(false);
          ...
          <table className="w-full">
            <thead className="bg-gray-100 border-b">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold">SubType ID</th>
            ...
            <tbody className="divide-y">
              {Array.isArray(getSortedRecords()) && getSortedRecords().map((record: CategorySubType, idx: number) => (
                <tr key={record.CategorySubTypeID ?? `${record.CategoryID}-${record.SubTypeID}-${idx}`} className="hover:bg-gray-50">
                  <td className="px-6 py-4">{record.CategoryID}</td>
                  <td className="px-6 py-4">{record.SubTypeID}</td>
                  <td className="px-6 py-4 text-right space-x-2">
                    <button
                      onClick={() => setEditingId(String(record.CategorySubTypeID))}
                    ...
                    <button
                      onClick={() => handleDelete(String(record.CategorySubTypeID))}
