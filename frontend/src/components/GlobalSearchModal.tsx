...
export default function GlobalSearchModal({ open, query, onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl max-h-[75vh] overflow-y-auto rounded-xl bg-white shadow-2xl">
                ...
                {data.items.map((item) => (
                  ...
                  >
                    <span className="font-medium text-slate-900">
                      {item.ItemName}
                      {item.ItemVersion ? <span className="ml-1 text-slate-500">({item.ItemVersion})</span> : null}
                    </span>
              ...
              <Section title="Miniatures" icon={<Layers className="h-4 w-4" />}>
                {data.miniatures.map((m) => (
                  <ResultLink
                    ...
                    onClose={onClose}
                  >
                    <span className="font-medium text-slate-900">{m.MiniatureName}</span>
                    <span className="mt-0.5 block text-xs text-slate-500">Qty: {m.MiniatureQuantity}</span>
                  </ResultLink>
                ))}
              </Section>
            )}
              ...
              <Section title="Publishers" icon={<Building2 className="h-4 w-4" />}>
                {data.publishers.map((p) => (
                  <ResultLink
                    ...
                    onClose={onClose}
                  >
                    <span className="font-medium text-slate-900">{p.PublisherName}</span>
                  </ResultLink>
                ))}
              </Section>
            )}
              ...
              <Section title="Collections" icon={<BookOpen className="h-4 w-4" />}>
                {data.collections.map((c) => (
                  <ResultLink
                    ...
                    onClose={onClose}
                  >
                    <span className="font-medium text-slate-900">{c.CollectionName}</span>
                    {c.CollectionTypeName && (
                      <span className="mt-0.5 block text-xs text-slate-500">{c.CollectionTypeName}</span>
                    )}
                  </ResultLink>
                ))}
              </Section>
              ...
              <Section title="Locations" icon={<MapPin className="h-4 w-4" />}>
                {data.locations.map((l) => (
                  <ResultLink
                    ...
                    onClose={onClose}
                  >
                    <span className="font-medium text-slate-900">{l.LocationName}</span>
                    {l.LocationTypeName && (
                      <span className="mt-0.5 block text-xs text-slate-500">{l.LocationTypeName}</span>
                    )}
                  </ResultLink>
                ))}
              </Section>
...
function Section({ title, icon, children }: { title: string; icon: ReactNode; children: ReactNode }) {
  return (
    <div>
      <div className="flex items-center gap-2 bg-slate-50 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
        {icon}
        {title}
      </div>
      <div>{children}</div>
...
function ResultLink({ to, onClose, children }: { to: string; onClose: () => void; children: ReactNode }) {
  return (
    <Link
      to={to}
      onClick={onClose}
      className="block px-4 py-2.5 text-sm transition hover:bg-sky-50"
    >
      {children}
    </Link>
  );
